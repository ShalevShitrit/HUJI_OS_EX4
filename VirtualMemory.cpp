          ...
          dfsRes *result, uint64_t path)
{
  ...
  for (int i = 0; i < PAGE_SIZE; ++i)
  {
    ...
    dfs (level + 1, page, child, parent, result, combine_binary (path, i));
  }

  if (empty_childs == PAGE_SIZE && frame != (word_t) parent
      && frame < result->empty_frame)
  {
